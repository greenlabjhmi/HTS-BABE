'''
This code is meant to create MA plots that are .pdf for manipulation for figures
08/22/17 Anthony Schuller
Reads in two wig files and outputs this plot
Same as hrf_seq_ratio.py except output is pdf
'''
...
def create_maplot (x_array, y_array, out_prefix):
	...
	plot.set_title("", fontsize = 8)
	plot.set_xlabel("", fontsize = 8)
	plot.set_ylabel("", fontsize = 8)
	plot.set_yscale('log')
	plot.set_xscale('log')
	plot.scatter(x_array, y_array, color='black', s=3)
	plot.set_xlim(0.01,100000)
	plot.set_ylim(.001,1000)
...
create_maplot (x_array, y_array, out_prefix)
